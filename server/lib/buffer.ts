...
import type { StorageRecord, ChartJson, UserSummaryJson } from './schemas'
...
export async function updateUserSummary(
  ...
  if (records.length === 0) return { date_counts: {}, total_submissions: 0 }
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    if (body) {
      summary = JSON.parse(decoder.decode(body)) as UserSummaryJson
    } else {
      summary = { date_counts: {}, total_submissions: 0 }
    }

    for (const r of records) {
      const dateStr = `${r.year}-${String(r.month).padStart(2, '0')}-${String(r.day).padStart(2, '0')}`
      if (!summary.date_counts[dateStr]) summary.date_counts[dateStr] = {}
      summary.date_counts[dateStr][r.model_id] =
        (summary.date_counts[dateStr][r.model_id] ?? 0) + 1
...
export async function readUserSummary(
  ...
  if (!body) return null
  return JSON.parse(decoder.decode(body)) as UserSummaryJson
}
...
export async function readChartJson(bucket: R2Bucket): Promise<ChartJson> {
  ...
  if (!body) return { data: {}, models: [], total_submissions: 0 }
...
function aggregateRecords(records: StorageRecord[], into: ChartJson): void {
  for (const r of records) {
    ...
    into.data[dateStr][r.model_id] =
      (into.data[dateStr][r.model_id] ?? 0) + 1
...
export async function rebuildChartJson(bucket: R2Bucket): Promise<void> {
  const chart: ChartJson = { data: {}, models: [], total_submissions: 0 }
